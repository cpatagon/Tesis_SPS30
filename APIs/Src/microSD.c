...
#define CHECK_ERROR(fresult, sd, msg)                                                              \
    do {                                                                                           \
        if ((fresult) != FR_OK) {                                                                  \
            send_uart((sd), (msg));                                                                \
...
#define FILE_CHECK_ERROR(fresult, sd, msg)                                                         \
    do {                                                                                           \
        if ((fresult) != FR_OK) {                                                                  \
            send_uart((sd), (msg));                                                                \
            send_uart((sd), get_fresult_message((fresult)));                                       \
...
const char * get_fresult_message(FRESULT fr) {
    ...
    case FR_OK:
        return FR_OK_MSG;
    case FR_DISK_ERR:
        return FR_DISK_ERR_MSG;
    case FR_INT_ERR:
        return FR_INT_ERR_MSG;
    case FR_NOT_READY:
        return FR_NOT_READY_MSG;
    case FR_NO_FILE:
        return FR_NO_FILE_MSG;
    case FR_NO_PATH:
        return FR_NO_PATH_MSG;
    case FR_INVALID_NAME:
        return FR_INVALID_NAME_MSG;
    case FR_DENIED:
        return FR_DENIED_MSG;
    case FR_EXIST:
        return FR_EXIST_MSG;
    case FR_INVALID_OBJECT:
        return FR_INVALID_OBJECT_MSG;
    case FR_WRITE_PROTECTED:
        return FR_WRITE_PROTECTED_MSG;
    case FR_INVALID_DRIVE:
        return FR_INVALID_DRIVE_MSG;
    case FR_NOT_ENABLED:
        return FR_NOT_ENABLED_MSG;
    case FR_NO_FILESYSTEM:
        return FR_NO_FILESYSTEM_MSG;
    case FR_MKFS_ABORTED:
        return FR_MKFS_ABORTED_MSG;
    case FR_TIMEOUT:
        return FR_TIMEOUT_MSG;
    case FR_LOCKED:
        return FR_LOCKED_MSG;
    case FR_NOT_ENOUGH_CORE:
        return FR_NOT_ENOUGH_CORE_MSG;
    case FR_TOO_MANY_OPEN_FILES:
        return FR_TOO_MANY_OPEN_FILES_MSG;
    default:
        return FR_UNKNOWN_ERROR_MSG;
    }
}
...
void microSD_getSize(MicroSD * sd) {
    // Obtiene el número de clústeres libres y el número total de clústeres en el sistema de
    // archivos.
    ...
    uint32_t total = (uint32_t)((sd->pfs->n_fatent - RESERVED_CLUSTERS) * sd->pfs->csize *
                                SECTORS_TO_KILOBYTES_CONVERSION_FACTOR);
    ...
    uint32_t freeSpace =
        (uint32_t)(sd->fre_clust * sd->pfs->csize * SECTORS_TO_KILOBYTES_CONVERSION_FACTOR);
